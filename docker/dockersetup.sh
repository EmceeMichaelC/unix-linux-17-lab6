...
docker build -t uladmin/rsyslog_agg:v2 .
...
docker run --name rsyslog -d -v /data/rsyslog:/data/rsyslog -p 514:514 -p 514:514/udp --init=true --init-path /usr/bin/docker-init --pid=host uladmin/rsyslog_agg:v2
